...
**Created**: 2026-03-30  
**Status**: Draft  
**Input**: User description: "Right now, I have a manual release CI/CD process. I want you to automate this, so a patch version is bumped every time I push to main, GitHub artifacts are built, and downstream package managers can publish after the successful release build."
...
1. **Given** a `main` commit that already has a reachable semver tag pointing at
   ...
   workflow reports that the commit is already released by tag and refreshes or
   recreates the matching GitHub release assets instead of calculating another
   version.
...
- **UX-005**: Local preview and CI automation MUST describe the behavior as a
  patch-release flow triggered by pushes to `main`, with downstream package
  publishers invoked only after the GitHub release step succeeds.
...
- **FR-013**: Downstream package-manager workflows MUST be invoked only after the
  upstream build and GitHub release steps succeed, and MUST consume the
  published release artifacts rather than rebuilding them independently.

# Implementation Plan: Automatic minor release automation
...
Convert the existing manual release flow into an automatic minor-release pipeline
...
verification, centralizes minor-version calculation and rerun safety in
`internal/release`, keeps GitHub release orchestration in
`.github/workflows/release.yml`, and lets downstream package-manager workflows
consume published releases.
...
minor preview and rerun guards, and unit tests for semver/release-state helpers
...
automation
**Performance Goals**: Minor preview completes within 1 second locally; the
...
**Scale/Scope**: One repository, one default branch (`main`), minor-release
automation, a supported release matrix of Linux amd64/arm64, macOS amd64/arm64,
and Windows amd64 artifacts, plus downstream package publishers that consume
published releases
...
`quickstart.md`, and the release contract define testable minor-release
automation, release-to-package-manager fan-out, race-safe publication, and
secret-safe operator messaging.
...
| performance-dependency-budget | PASS   | Minor-release automation reuses existing GitHub tooling and version metadata, avoids new runtime dependencies, and keeps the workflow within the existing release budget.                    |
...
**Structure Decision**: Keep version calculation, tag filtering, branch
validation, and rerun safety in `internal/release`; keep local preview and
operator-facing CLI glue in `scripts/nextversion`; keep GitHub release build and
publication in `.github/workflows/release.yml`; keep downstream packaging in
release-triggered workflows such as `.github/workflows/aur-publish.yml`; and
reuse the existing embedded metadata in `pkg/dotenvsync` only for artifact
verification.

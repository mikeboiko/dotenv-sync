...
verification, centralizes patch-version calculation and rerun safety in
...
consume published releases through direct reusable-workflow fan-out.
...
**Scale/Scope**: One repository, one default branch (`main`), patch-release
...
`quickstart.md`, and the release contract define testable patch-release
...
| performance-dependency-budget | PASS   | Patch-release automation reuses existing GitHub tooling and version metadata, avoids new runtime dependencies, and keeps the workflow within the existing release budget.                    |
...
reusable workflows such as `.github/workflows/aur-publish.yml`; and

...
func newDoctorCommand(s streams, opts *rootOptions) *cobra.Command {
	return &cobra.Command{
		Use:   "doctor",
		Short: "Check config and provider prerequisites",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return report.NewAppError("E003", report.ExitOperational, "provider readiness failed", "provider-backed commands cannot continue", "check the configured provider and retry", err)
			}
			if status.Code != "" {
